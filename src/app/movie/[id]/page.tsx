...
export default function MovieDetails({ params }: { params: { id: string } }) {
    ...
    const fetchMovieDetails = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.themoviedb.org/3/movie/${params.id}?api_key=${API_KEY}&append_to_response=credits,similar`
        );
        ...
        const savedRating = localStorage.getItem(`movie_rating_${params.id}`);
        if (savedRating) {
  ...
  const handleRating = (rating: number) => {
    ...
    localStorage.setItem(`movie_rating_${params.id}`, rating.toString());
  };
